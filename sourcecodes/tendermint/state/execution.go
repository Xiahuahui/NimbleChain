...
func (blockExec *BlockExecutor) CreateProposalBlock(
	...
	return state.MakeBlock(height, delay_time, deliver_time, txs, commit, evidence, proposerAddr)
}
...
func (blockExec *BlockExecutor) ApplyBlock(
	...
	endTime1 := time.Now()
	fmt.Println("区块验证时间：", endTime1.Sub(startTime1).Seconds())
...
func execBlockOnProxyApp(
	...
	for testi := 0; testi < len(block.Txs); testi++ {
		txRes := abci.ResponseDeliverTx{Code: abci.CodeTypeOK}
		abciResponses.DeliverTxs[testi] = &txRes
		validTxs++
	}
