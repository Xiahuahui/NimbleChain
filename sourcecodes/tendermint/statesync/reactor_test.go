...
func TestReactor_Receive_ChunkRequest(t *testing.T) {
	...
	for name, tc := range testcases {
		...
		t.Run(name, func(t *testing.T) {
			...
			// Start a reactor and send.sh a ssproto.ChunkRequest, then wait for and check response
...
func TestReactor_Receive_SnapshotsRequest(t *testing.T) {
	...
	for name, tc := range testcases {
		...
		t.Run(name, func(t *testing.T) {
			...
			// Start a reactor and send.sh a SnapshotsRequestMessage, then wait for and check responses
			cfg := config.DefaultStateSyncConfig()
			r := NewReactor(*cfg, conn, nil, "")
			err := r.Start()
			require.NoError(t, err)
