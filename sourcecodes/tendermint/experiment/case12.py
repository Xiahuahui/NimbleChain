...
if __name__ == '__main__':
    ...
    ***************************模拟生成数据***************************
    ...
    ax.plot(x_list,tps_list,label='Normal',color='#4682B4',marker='^',markersize=10,linewidth=5)
    # ax.plot(x_list,tps_list2,label='25%Crash',color='#00008B',marker='s',markersize=15,linewidth=5)
    ax.set_ylabel('TPS',fontsize=20)
    ...
    #     label.set_fontweight('bold')     # 设置字体加粗
    ax.set_xlabel('timeout threshold(ms)',fontsize=24)
    ...
    # 保存图表为文件
    plt.savefig('fig_observation_timeout_normal.pdf', dpi=300, bbox_inches='tight')  # 保存高 DPI 的 PDF 文件
    plt.show()
