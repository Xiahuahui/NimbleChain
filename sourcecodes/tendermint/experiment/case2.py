...
def rtt(sample, RCPT, RCPTVAR, alpha, beta,K):  # rtt是根据五个变量预测下一个时间的值
    # logging.info("data:%s RCPT:%s RCPTVAR:%s ",sample,RCPT,RCPTVAR)
    ...
    timeout = RCPT
    # logging.info("data:%s timeout:%s RCPT:%s RCPTVAR:%s timeVar:%s",sample,timeout,RCPT,RCPTVAR,abs(timeVar))
...
# 激励函数的作用,这个是和TPS正相关的一个量
# bct_data表示的真实的bct
# timeout_data表示的预测值timeout
# 其实我们想要的激励函数是TPs的大小
# TODO 解释一下这个函数 这个函数为什么要这么实现,这个要仔细检查一下是个错的目前
def reward(bct_data, timeout_data):
    assert (len(bct_data) == len(timeout_data))
    reward_list = []
    view_change_list = []
...
def print_diff_para(start_index,end_index,low,mid,high,best,bct_data,low_timeout_list,mid_timeout_list,high_timeout_list,best_timeout_list,best_para_list):
    ...
    ax1.set_yticks(np.arange(500, 3500, 500))  # 每隔 0.06 设置一个刻度
    ...
    ax1.set_xticks(np.arange(0, 180,30))
    ax1.tick_params(axis='y',labelsize=30)
    ax1.tick_params(axis='x',labelsize=30)
    ax1.set_xlabel('Block Index',fontsize=30)
    # ax1.legend()
