...
def adjust(fixed_data,leader_partion,flag,init_timeout):
    byz_bct_list = []
    tps = []
    vc_num = 0
    fixed_timeout_list = []
    ...
    for bct in fixed_data:
        if fixed_index % leader_partion == 0:
            if flag == 'Fixed':
                bct = bct + 1000.0
            else:
                bct = fixed_timeout_list[-1]
        byz_bct_list.append(bct)
        pdt = bct
        if bct > fixed_timeout_list[-1]:
            vc_num = vc_num + 1
            pdt = bct *2
...
if __name__ == '__main__':
    ...
    ***************************模拟生成数据***************************
    '''
    # config = Config(k=30,high=780,low=380,high_std=30,low_std=20,step=5)
    ...
    fixed_data = data[:150]
    timeout3,tps1,vc_num1,byz_bct_list = adjust(fixed_data,4,"Jac",3000.0)
    print("byz_bct_list: ",byz_bct_list)
    # timeout4,tps2,vc_num2,byz_bct_list2 = adjust(fixed_data,888888888888,"Fixed",1000.0)
    # timeout5 = adjust(fixed_data,5,"Fixed",3000.0)
    # timeout6 = adjust(fixed_data,6)
    # timeout7 = adjust(fixed_data,4,"Fixed",3000.0)
    ...
    #     ax1.plot(x[i:i+2], best[start_index+i:start_index+i+2],color=colors[i],linewidth=3)
    # ax1.plot([], [], color='gray', label='best',linewidth=3)
    # ax1.plot(tps_list)
    ax1.set_ylabel('TPS',fontsize=30)
    ax1.tick_params(axis='y',labelsize=30)
    ax2.tick_params(axis='x',labelsize=30)
    ax2.set_xlabel('Timeout(ms)',fontsize=30)
    ax1.legend()
    ...
    ax2.plot(fixed_data, label='PDT',linewidth=3)
    ...
    ax2.set_ylabel('PDT (ms)', color='green',fontsize=30)
