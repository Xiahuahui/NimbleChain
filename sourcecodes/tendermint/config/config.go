...
type ConsensusConfig struct {
	...
	ProposeFilePath       string        `mapstructure:"propose_file_path"`
	LatencyFilePath       string        `mapstructure:"latency_file_path"`
	RoundFilePath         string        `mapstructure:"round_file_path"`
	CostFilePath          string        `mapstructure:"cost_file_path"`
	PdtFilePath           string        `mapstructure:"pdt_file_path"`
	BctFilePath           string        `mapstructure:"bct_file_path"`
	CreateBlockFilePath   string        `mapstructure:"create_block_file_path"`
	Update                bool          `mapstructure:"update"` //要不要更新timeout
	ByzantineNodeNum      int64         `mapstructure:"byzantine_node_num "`
	ByzantineNodeList     []int         `mapstructure:"byzantine_node_list "`
	CrashNodeList         []int         `mapstructure:"crash_node_list "`
	CurrentProposeTime    time.Duration `mapstructure:"current_propose_time"`
	UpdateLevel           string        `mapstructure:"update_level"`
	CC                    bool          `mapstructure:"c_c"` //要不要测试拥塞
	MI                    int64         `mapstructure:"m_i"`
	SmallMI               int64         `mapstructure:"s_m_i"`
	EpochSizeUpdate       bool          `mapstructure:"epoch_size_update"` //要不要更新
	K                     int64         `mapstructure:"k"`
	KUpdate               bool          `mapstructure:"k_update"`
	ProposeTimeoutFlag    string        `mapstructure:"propose_timeout_flag"`
	PingNode              string        `mapstructure:"ping_node"`
	Method                string        `mapstructure:"method"`
	Version               string        `mapstructure:"version"`
	ParaA                 float64       `mapstructure:"para_a"`
}
...
func DefaultConsensusConfig() *ConsensusConfig {
	ok := true
	//ok := false
	var path1, path2, path3, path4, path5, path6, path7, id, pingnode string
	if ok {
		id = os.Getenv("ID")
		path1 = "../config_path/node" + id + "/propose_time.csv"
		path2 = "../config_path/node" + id + "/latency_time.csv"
		path3 = "../config_path/node" + id + "/commit_round.csv"
		path4 = "../config_path/node" + id + "/cost_time.csv"
		path5 = "../config_path/node" + id + "/pdt.txt"
		path6 = "../config_path/node" + id + "/bct.txt"
		path7 = "../config_path/ping-result/sequence.csv"

	} else {
		id = ""
		path1 = "propose_time.csv"
		path2 = "latency_time.csv"
		path3 = "commit_round.csv"
		path4 = "cost_time.csv"
		path5 = "pdt.txt"
		path6 = "pdt.txt"
		path7 = "sequence.csv"
	...
	if id == "" {
		pingnode = "www.baidui.com"
	} else {
		int_id, _ := strconv.Atoi(id)
		pingid := (int_id + 1) % 4 //ping node
		pingnode = "node" + strconv.Itoa(pingid)
	}
	...
	return &ConsensusConfig{
		...
		TimeoutPrevote:              30000 * time.Millisecond,
		TimeoutPrevoteDelta:         500 * time.Millisecond,
		TimeoutPrecommit:            30000 * time.Millisecond,
		...
		PeerQueryMaj23SleepDuration: 500 * time.Millisecond,
		DoubleSignCheckHeight:       int64(0),
		Epoch:                       int64(1), //当为block-base时=1  //这个参数要每次都要指定 而不是block时就不用管了
		ProposeFilePath:             path1,
		LatencyFilePath:             path2,
		RoundFilePath:               path3,
		CostFilePath:                path4,
		PdtFilePath:                 path5,
		BctFilePath:                 path6,
		CreateBlockFilePath:         path7,
		Update:                      true,
		ByzantineNodeNum:            int64(0), //1{6},2{6,12},3{6,12,15} 4{6,10,12,15},5{6,8,10,13,15} //1{6}2{6,7}
		ByzantineNodeList:           []int{},
		CrashNodeList:               []int{0, 3, 6, 9, 12, 15, 18, 22, 26, 29},
		CurrentProposeTime:          5000 * time.Millisecond,
		UpdateLevel:                 "block", //[epoch, block, merge]
		CC:                          false,
		MI:                          int64(400000), //TODO 当前默认MI是SmallMI的整数倍。
		SmallMI:                     int64(80000),
		K:                           int64(4),
		EpochSizeUpdate:             false,
		KUpdate:                     false,
		ProposeTimeoutFlag:          "Exponential Backoff",
		PingNode:                    pingnode,
		Method:                      "EMA",
		Version:                     "v1",
		ParaA:                       float64(-2),
...
func (cfg *ConsensusConfig) Propose(round int32) time.Duration {
	if cfg.ProposeTimeoutFlag == "Exponential Backoff" {
		return time.Duration(
			cfg.TimeoutPropose.Nanoseconds()*int64(math.Pow(float64(2), float64(round))),
		) * time.Nanosecond
	}
	return time.Duration(
