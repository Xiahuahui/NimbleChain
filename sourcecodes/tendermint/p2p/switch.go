...
// SwitchFilterTimeout sets the proposetimeout used for peer filters.
...
// BroadcastEnvelope runs a go routine for each attempted send.sh, which will block trying
// to send.sh for defaultSendTimeoutSeconds. Returns a channel which receives
// success values for each attempted send.sh (false if times out). Channel will be
...
func (sw *Switch) BroadcastEnvelope(e Envelope) chan bool {
	if e.ChannelID == byte(0x24) {
		sw.Logger.Info("Broadcast", "channel", e.ChannelID)
	}
...
// Broadcast runs a go routine for each attempted send.sh, which will block trying
// to send.sh for defaultSendTimeoutSeconds. Returns a channel which receives
// success values for each attempted send.sh (false if times out). Channel will be
// closed once msg bytes are sent to all peers (or time out).
...
func (sw *Switch) addPeer(p Peer) error {
	...
	// Start the peer's send.sh/recv routines.
