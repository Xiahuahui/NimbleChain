...
func SendEnvelopeShim(p Peer, e Envelope, lg log.Logger) bool {
	...
	if err != nil {
		lg.Error("marshaling message to send.sh", "error", err)
		return false
	}
	if e.ChannelID == byte(0x24) {
		lg.Info("the msgbytes", msgBytes)
	}
...
func (p *peer) SendEnvelope(e Envelope) bool {
	if !p.IsRunning() {
		...
		if e.ChannelID == byte(0x24) {
			p.Logger.Info("ferereteteteteter", "chid", p.channels)
		}
		return false
	}
	msg := e.Message
	...
	msgBytes, err := proto.Marshal(msg)
	if err != nil {
		p.Logger.Error("marshaling message to send.sh", "error", err)
		return false
	}
...
// send.sh queue is full after timeout, specified by MConnection.
// SendEnvelope replaces Send which will be deprecated in a future release.
func (p *peer) Send(chID byte, msgBytes []byte) bool {
	if chID == byte(0x24) {
		p.Logger.Info("dfdfdfdfdfdfdfdfdfdfdfdfdfdfsend")
	}
...
// false if the send.sh queue is full.
...
// CanSend returns true if the send.sh queue is not full, false otherwise.
