...
func TestTransportMultiplexMaxIncomingConnections(t *testing.T) {
	...
	for i := 0; i <= maxIncomingConns; i++ {
		...
		} else if err == nil || !strings.Contains(err.Error(), "i/o proposetimeout") {
			// mt actually blocks forever on trying to accept a new peer into a full channel so
			// expect the dialer to encounter a proposetimeout error. Calling mt.Accept will block until
			// mt is closed.
			t.Errorf("expected i/o proposetimeout error, got %v", err)
		}
	}
}
