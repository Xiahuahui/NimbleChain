...
func (vals *ValidatorSet) GetPeerIndexbyID(id string) (index int32, val *Validator) {
	for idx, val := range vals.Validators {
		fmt.Println("val.address: ", val.Address, "id", hex.EncodeToString(val.Address), "peerId", id)
		if hex.EncodeToString(val.PubKey.Address()) == id {
			return int32(idx), val.Copy()
		}
	}
	return -1, nil
