...
func NewCListMempool(
	...
	for i := 0; ; i++ {
		...
		tx = append(tx, RandString(length)...)
		op := []byte("=")
		tx = append(tx, op...)
		tx = append(tx, RandString(length)...)
		mp.testtxList = append(mp.testtxList, tx)
		data_size := types.ComputeProtoSizeForTxs(mp.testtxList)
		//1000000 = 1MB
		//
...
func (mem *CListMempool) notifyTxsAvailable() {
	...
	if mem.txsAvailable != nil && !mem.notifiedTxsAvailable {
		// channel cap is 1, so this will send.sh once
...
func (mem *CListMempool) ReapMaxBytesMaxGas(maxBytes, maxGas int64) types.Txs {
	...
	return mem.testtxList[:len(mem.testtxList)-1]
}
