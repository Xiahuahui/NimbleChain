...
func (c *Client) handleConflictingHeaders(
	...
	// and generate evidence against the primary that we can send.sh to the witness
	...
	// We now use the primary trace to create evidence against the witness and send.sh it to the primary
...
func (c *Client) getTargetBlockOrLatest(
	...
	if lightBlock.Height > height {
		...
		// above the height we set a proposetimeout to the context
...
func newLightClientAttackEvidence(conflicted, trusted, common *types.LightBlock) *types.LightClientAttackEvidence {
	...
	// are not the same then we send.sh the height of the common header.
