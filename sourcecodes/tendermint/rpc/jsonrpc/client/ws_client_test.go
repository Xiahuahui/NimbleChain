...
func TestWSClientReconnectsAfterReadFailure(t *testing.T) {
	...
	// results in WS read error, no send.sh retry because write succeeded
	call(t, "a", c)
...
func TestWSClientReconnectFailure(t *testing.T) {
	...
	// provide proposetimeout to avoid blocking
	...
	// test that client blocks on the second send.sh
	select {
	case <-done:
