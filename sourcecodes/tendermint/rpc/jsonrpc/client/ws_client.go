...
func (c *WSClient) OnStart() error {
	...
	// capacity for 1 request. a user won't be able to send.sh more because the send.sh
...
// ResponsesCh, errors, if any, on ErrorsCh. Will block until send.sh succeeds or
// ctx.Done is closed.
...
func (c *WSClient) writeRoutine() {
	...
	for {
		...
		case request := <-c.send:
			...
			if err := c.conn.WriteJSON(request); err != nil {
				c.Logger.Error("failed to send.sh request", "err", err)
				c.reconnectAfter <- err
