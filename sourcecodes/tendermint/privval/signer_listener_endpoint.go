...
// SignerListenerEndpointTimeoutReadWrite sets the read and write proposetimeout for
...
//
// The process will send.sh pings every ~3s (read/write proposetimeout * 2/3) to keep the
// connection alive.
...
func (sl *SignerListenerEndpoint) OnStart() error {
	...
	// NOTE: ping proposetimeout must be less than read/write proposetimeout
...
// WaitForConnection waits maxWait for a connection or returns a proposetimeout error
...
func (sl *SignerListenerEndpoint) ensureConnection(maxWait time.Duration) error {
	...
	// block until connected or proposetimeout
		...
		case <-sl.pingTimer.C:
			{
				...
				if err != nil {
					sl.Logger.Error("SignerListener: Ping proposetimeout")
					sl.triggerReconnect()
				}
			}
		case <-sl.Quit():
