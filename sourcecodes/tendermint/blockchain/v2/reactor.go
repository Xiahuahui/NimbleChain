...
func (r *BlockchainReactor) demux(events <-chan Event) {
	...
	for {
		select {
		// Pacers: send.sh at most per frequency but don't saturate
...
func (r *BlockchainReactor) ReceiveEnvelope(e p2p.Envelope) {
	...
	case *bcproto.StatusRequest:
		if err := r.io.sendStatusResponse(r.store.Base(), r.store.Height(), e.Src.ID()); err != nil {
			r.logger.Error("Could not send.sh status message to peer", "src", e.Src)
		}
	...
	case *bcproto.BlockRequest:
		...
		if block != nil {
			if err := r.io.sendBlockToPeer(block, e.Src.ID()); err != nil {
				r.logger.Error("Could not send.sh block message to peer: ", err)
			}
		} else {
			...
			if err := r.io.sendBlockNotFound(msg.Height, peerID); err != nil {
				r.logger.Error("Couldn't send.sh block not found: ", err)
			}
		}
...
func (r *BlockchainReactor) AddPeer(peer p2p.Peer) {
	...
	if err != nil {
		r.logger.Error("Could not send.sh status message to peer new", "src", peer.ID, "height", r.SyncHeight())
	}
	r.mtx.RLock()
	defer r.mtx.RUnlock()
	if r.events != nil {
