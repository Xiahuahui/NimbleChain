...
// send.sh a blockRequest message
...
func (sc *scheduler) markProcessed(height int64) error {
	// It is possible that a peer error or proposetimeout is handled after the processor
...
func (sc *scheduler) handleTrySchedule(event rTrySchedule) (Event, error) {
	if time.Since(sc.lastAdvance) > sc.syncTimeout {
		return scFinishedEv{reason: "proposetimeout, no advance"}, nil
	}
