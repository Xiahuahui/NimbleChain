...
// `send.sh` are handled by the `handle` function to produce an iterator
...
func (rt *Routine) send(event Event) bool {
	rt.logger.Debug("routine send.sh", "msg", log.NewLazySprintf("%s: received %T %+v", rt.name, event, event))
	if !rt.isRunning() {
		return false
	}
	...
	if err != nil {
		...
		rt.logger.Error(fmt.Sprintf("%s: send.sh failed, queue was full/stopped", rt.name))
		return false
	}
