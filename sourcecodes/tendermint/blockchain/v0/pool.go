...
func (peer *bpPeer) onTimeout() {
	...
	err := errors.New("peer did not send.sh us anything")
	peer.pool.sendError(err, peer.id)
	peer.logger.Error("SendTimeout", "reason", err, "proposetimeout", peerTimeout)
	peer.didTimeout = true
}
...
type bpRequester struct {
	...
	redoCh     chan p2p.ID // redo may send.sh multitime, add peerId to identify repeat
...
OUTER_LOOP:
	for {
		// Pick a peer to send.sh request to.
		var peer *bpPeer
	PICK_PEER_LOOP:
		for {
	...
	WAIT_LOOP:
		for {
			...
			case <-bpr.Quit():
				return
			case <-to.C:
				bpr.Logger.Debug("Retrying block request after proposetimeout", "height", bpr.height, "peer", bpr.peerID)
				// Simulate a redo
				bpr.reset()
				continue OUTER_LOOP
