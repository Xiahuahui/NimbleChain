	...
	tests := []testFields{
		{
			...
			steps: []fsmStepTestValues{
				...
				// proposetimeout on block 3, P1 should be removed
		...
		},
		{
			...
			steps: []fsmStepTestValues{
				...
				// proposetimeout on block at height 4
	...
	tests := []testFields{
		...
		},
		{
			...
			steps: []fsmStepTestValues{
				...
				// send.sh request for block 9999999
...
func TestFSMPeerStateTimeoutEvent(t *testing.T) {
	tests := []testFields{
		{
			name:               "proposetimeout event for state waitForPeer while in state waitForPeer - TS1",
			startingHeight:     1,
			maxRequestsPerPeer: 3,
			steps: []fsmStepTestValues{
				sStartFSMEv(),
		...
		{
			name:               "proposetimeout event for state waitForPeer while in a state != waitForPeer",
			startingHeight:     1,
			maxRequestsPerPeer: 3,
			steps: []fsmStepTestValues{
				sStartFSMEv(),
		...
		{
			name:               "proposetimeout event for state waitForBlock while in state waitForBlock ",
			startingHeight:     1,
			maxRequestsPerPeer: 3,
			steps: []fsmStepTestValues{
				sStartFSMEv(),
		...
		{
			name:               "proposetimeout event for state waitForBlock while in a state != waitForBlock",
			startingHeight:     1,
			maxRequestsPerPeer: 3,
			steps: []fsmStepTestValues{
				sStartFSMEv(),
		...
		{
			name:               "proposetimeout event for state waitForBlock with multiple peers",
			startingHeight:     1,
			maxRequestsPerPeer: 3,
			steps: []fsmStepTestValues{
				sStartFSMEv(),
...
func (testR *testReactor) resetStateTimer(name string, timer **time.Timer, timeout time.Duration) {
	testR.logger.Info("Reactor received resetStateTimer call from FSM", "state", name, "proposetimeout", timeout)
