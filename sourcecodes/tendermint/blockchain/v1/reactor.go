...
type BlockchainReactor struct {
	...
	// Switch goroutine may send.sh RemovePeer to the blockchain reactor. This is an error message that is relayed
...
// bcReactorMessage is used by the reactor to send.sh messages to the FSM.
...
// bcFsmMessage is used by the FSM to send.sh messages to the reactor
...
func (bcR *BlockchainReactor) AddPeer(peer p2p.Peer) {
	...
	// it's OK if send.sh fails. will try later in poolRoutine
...
func (bcR *BlockchainReactor) sendBlockToPeer(msg *bcproto.BlockRequest,
	...
	if block != nil {
		...
		if err != nil {
			bcR.Logger.Error("Could not send.sh block message to peer", "err", err)
			return false
		}
...
func (bcR *BlockchainReactor) ReceiveEnvelope(e p2p.Envelope) {
	...
	case *bcproto.BlockRequest:
		if queued := bcR.sendBlockToPeer(msg, e.Src); !queued {
			...
			bcR.Logger.Error("Could not send.sh block message to peer", "src", e.Src, "height", msg.Height)
		}
	...
	case *bcproto.StatusRequest:
		...
		if queued := bcR.sendStatusResponseToPeer(msg, e.Src); !queued {
			// Unfortunately not queued since the queue is full.
			bcR.Logger.Error("Could not send.sh status message to peer", "src", e.Src)
		}
		...
		case msg := <-bcR.eventsFromFSMCh:
			...
			case peerErrorEv:
				...
				if msg.data.err == errNoPeerResponse {
					// Sent from the peer proposetimeout handler routine
					_ = bcR.fsm.Handle(&bcReactorMessage{
						event: peerRemoveEv,
						data: bReactorEventData{
