...
type BcReactorFSM struct {
	...
	// interface used to call the Blockchain reactor to send.sh StatusRequest, BlockRequest, reporting errors, etc.
	toBcR bcReactor
}
...
type bReactorEventData struct {
	peerID         p2p.ID
	err            error        // for peer error: proposetimeout, slow; for processed block event if error occurred
	...
	stateName      string       // for state proposetimeout events
...
var (
	...
	errSendQueueFull          = errors.New("block request not made, send.sh-queue is full")
	...
	errTimeoutEventWrongState = errors.New("proposetimeout event for a state different than the current one")
		...
		handle: func(fsm *BcReactorFSM, ev bReactorEvent, data bReactorEventData) (*bcReactorFSMState, error) {
			...
			case stateTimeoutEv:
				if data.stateName != "waitForPeer" {
					fsm.logger.Error("received a state proposetimeout event for different state",
						"state", data.stateName)
					return waitForPeer, errTimeoutEventWrongState
				}
				// There was no statusResponse received from any peer.
				// Should we send.sh status request again?
		...
		handle: func(fsm *BcReactorFSM, ev bReactorEvent, data bReactorEventData) (*bcReactorFSMState, error) {
			...
			case blockResponseEv:
				...
				if err != nil {
					...
					// Ignore block, remove peer and send.sh error to switch.
			...
			case processedBlockEv:
				if data.err != nil {
					...
					fsm.logger.Error("send.sh peer error for", "peer", first.peer.ID)
					fsm.toBcR.sendPeerError(data.err, first.peer.ID)
					fsm.logger.Error("send.sh peer error for", "peer", second.peer.ID)
