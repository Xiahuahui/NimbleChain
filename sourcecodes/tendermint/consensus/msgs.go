...
func MsgToProto(msg Message) (*tmcons.Message, error) {
	...
	switch msg := msg.(type) {
	case *PDTMessage:
		ptd := msg.pdt_indfo.ToProto()
		m := &tmcons.PDT{PdtInfo: ptd}
...
func MsgFromProto(p *tmcons.Message) (Message, error) {
	...
	case *tmcons.PDT:
		pdt, err := types.PDTInfoFromProto(msg.PdtInfo)
		if err != nil {
			return nil, fmt.Errorf("pdt msg to proto error: %w", err)
		}
		pb = &PDTMessage{pdt_indfo: pdt}
