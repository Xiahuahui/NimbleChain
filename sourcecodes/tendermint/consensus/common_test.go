...
//-------------------------------------------------------------------------------
// Functions for transitioning the consensus state 共识中的状态转换

func startTestRound(cs *State, height int64, round int32) {
	cs.enterNewRound(height, round)
	cs.startRoutines(0) //启动两个接收的线程
}
...
func decideProposal(
	...
	validRound := cs1.ValidRound //这个有效的轮次是什么意思。
...
func subscribeToVoter(cs *State, addr []byte) <-chan tmpubsub.Message {
	...
	ch := make(chan tmpubsub.Message) //这个是一个无缓冲的消息队列，用于接收订阅的消息。
	go func() {
...
// -------------------------------------------------------------------------------
// 这个是在timeout（200ms）的时间段内，确保没有新的恶事件发生。（下面应该是几个订阅器的输出）
...
// color for each validator ("validator" key must exist). //产生日志的函数
...
func randGenesisDoc(numValidators int, randPower bool, minPower int64) (*types.GenesisDoc, []types.PrivValidator) {
	validators := make([]types.GenesisValidator, numValidators)  //总体的validator
	privValidators := make([]types.PrivValidator, numValidators) //只是参与共识的valdator？
...
// and only once if onlyOnce=true 这个是个假的
