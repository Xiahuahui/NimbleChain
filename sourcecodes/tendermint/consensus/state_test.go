...
x * TestEnterProposeNoValidator - proposetimeout into prevote round
...
func TestStateProposerSelection0(t *testing.T) {
	...
	newRoundCh := subscribe(cs1.eventBus, types.EventQueryNewRound)         //检查是否收到了进入新的轮次的通道，
	proposalCh := subscribe(cs1.eventBus, types.EventQueryCompleteProposal) //检查是否收到了新的提案的通道

	startTestRound(cs1, height, round) //进入新的轮次、启动接收进程

	// Wait for new round so proposer is set.
	ensureNewRound(newRoundCh, height, round) //等待发送事件
...
// a non-validator should proposetimeout into the prevote round
func TestStateEnterProposeNoPrivValidator(t *testing.T) {
	cs, _ := randState(1)
	cs.SetPrivValidator(nil)
	height, round := cs.Height, cs.Round

	// Listen for propose proposetimeout event
	timeoutCh := subscribe(cs.eventBus, types.EventQueryTimeoutPropose)

	startTestRound(cs, height, round)

	// if we're not a validator, EnterPropose should proposetimeout
...
// a validator should not proposetimeout of the prevote round (TODO: unless the block is really big!)
func TestStateEnterProposeYesPrivValidator(t *testing.T) {
	...
	// if we're a validator, enterPropose should not proposetimeout
...
func TestStateOversizedBlock(t *testing.T) {
	...
	// and then should send.sh nil prevote and precommit regardless of whether other validators prevote and
	// precommit on it
	ensurePrevote(voteCh, height, round)
	validatePrevote(t, cs1, round, vss[0], nil)
	signAddVotes(cs1, tmproto.PrevoteType, propBlock.Hash(), propBlock.MakePartSet(partSize).Header(), vs2)
...
func TestStateLockNoPOL(t *testing.T) {
	...
	*/
	...
	// but with invalid args. then we enterPrecommitWait, and the proposetimeout to new round
	...
	*/
	...
	// now we're on a new round and not the proposer, so wait for proposetimeout
	ensureNewTimeout(timeoutProposeCh, height, round, cs1.config.Propose(round).Nanoseconds())
	...
	// and then prevote wait, which should proposetimeout. then wait for precommit
	...
	// then we enterPrecommitWait and proposetimeout into NewRound
	ensureNewTimeout(timeoutWaitCh, height, round, cs1.config.Precommit(round).Nanoseconds())
...
func TestStateLockPOLRelock(t *testing.T) {
	...
	*/
	...
	// before we proposetimeout to the new round set the new proposal
...
func TestStateLockPOLSafety1(t *testing.T) {
	...
	/*Round2
	// we proposetimeout and prevote our lock
...
func TestStateLockPOLSafety2(t *testing.T) {
	...
	// proposetimeout of precommit wait to new round
	ensureNewTimeout(timeoutWaitCh, height, round, cs1.config.Precommit(round).Nanoseconds())
...
func TestSetValidBlockOnDelayedPrevote(t *testing.T) {
	...
	// vs2 send.sh (delayed) prevote for propBlock
...
func TestSetValidBlockOnDelayedProposal(t *testing.T) {
	...
	// vs2, vs3 and vs4 send.sh prevote for propBlock
	signAddVotes(cs1, tmproto.PrevoteType, propBlockHash, propBlockParts.Header(), vs2, vs3, vs4)
	ensureNewValidBlock(validBlockCh, height, round)
...
func TestEmitNewValidBlockEventOnCommitWithoutBlock(t *testing.T) {
	...
	// vs2, vs3 and vs4 send.sh precommit for propBlock
...
func TestCommitFromPreviousRound(t *testing.T) {
	...
	// vs2, vs3 and vs4 send.sh precommit for propBlock for the previous round
	signAddVotes(cs1, tmproto.PrecommitType, propBlockHash, propBlockParts.Header(), vs2, vs3, vs4)
...
func TestStartNextHeightCorrectlyAfterTimeout(t *testing.T) {
	...
	// wait till proposetimeout occurs
...
func TestStateSlashingPrevotes(t *testing.T) {
	...
	// NOTE: we have to send.sh the vote for different block first so we don't just go into precommit round right
