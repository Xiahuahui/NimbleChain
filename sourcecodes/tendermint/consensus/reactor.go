...
const (
	StateChannel       = byte(0x20) //状态的通道（同步HRS这些消息）
	DataChannel        = byte(0x21) //数据的通道 （同步区块）
	VoteChannel        = byte(0x22) //投票的通道  （同步投票）
	VoteSetBitsChannel = byte(0x23) //投票状态的通道 （同步投票的状态）
	PDTChannel         = byte(0x24) //广播PDT的信息

	maxMsgSize = 1048576 // 1MB; NOTE/TODO: keep in sync with types.PartSet sizes.

	blocksToContributeToBecomeGoodPeer = 10000 //成为block好的peer节点的标准
	votesToContributeToBecomeGoodPeer  = 10000 //成为vote好的peer节点的标准
)
...
type Reactor struct {
	...
	mtx      tmsync.RWMutex
	waitSync bool
	eventBus *types.EventBus     //这个更像是纵向的
	rs       *cstypes.RoundState //状态
...
func (conR *Reactor) OnStart() error {
	...
	go conR.peerStatsRoutine() //统计当前节点收到的相应节点的消息

	conR.subscribeToBroadcastEvents()
	go conR.updateRoundStateRoutine() //100微秒更新一次状态，
...
func (conR *Reactor) SwitchToConsensus(state sm.State, skipWAL bool) {
	...
	// NewRoundStepMessage.
	conR.conS.updateToState(state) //这的状态更新应该是比较慢的。
...
// GetChannels implements Reactor //现在的理解为优先级越高，发送当前channel中消息的概率更大。
func (conR *Reactor) GetChannels() []*p2p.ChannelDescriptor {
	...
	return []*p2p.ChannelDescriptor{
		...
		},
		{
			...
			// once we gossip the whole block there's nothing left to send.sh until next height or round
			Priority:            10,
			SendQueueCapacity:   100,
...
func (conR *Reactor) AddPeer(peer p2p.Peer) {
	fmt.Println("peer.address", peer.NodeInfo())
...
func (conR *Reactor) ReceiveEnvelope(e p2p.Envelope) {
	...
	//if e.ChannelID == byte(0x24) {
	//	conR.Logger.Info("ReceivePDTEnvelope")
	//}
	if err != nil {
	...
	switch e.ChannelID {
	case StateChannel:
		conR.Logger.Debug("receive a state message form peer")
		switch msg := msg.(type) {
		...
		case *NewRoundStepMessage:
			...
			conR.Logger.Debug("recevice state msg ", "Height ", msg.Height, "Round ", msg.Round, "Step ", msg.Step, "peerId", e.Src.ID())
	...
	case DataChannel:
		conR.Logger.Debug("receive a data message form peer")
		if conR.WaitSync() {
		...
		switch msg := msg.(type) {
		case *PDTMessage:
			height, pdt_value, bct_value := msg.pdt_indfo.GetValue()
			if height != conR.conS.Height {
				conR.Logger.Error("the pdt height mismacth", "pdt.height", height, "cs.height", conR.conS.Height)
			}
			conR.Logger.Info("the pdt info", "height", height, "pdt_value", pdt_value, "bct_value", bct_value, "the validator", e.Src.String())
			conR.conS.validatorPDT[e.Src.String()] = pdt_value
	...
	case VoteChannel:
		conR.Logger.Debug("receive a vote message form peer")
		if conR.WaitSync() {
			conR.Logger.Info("Ignoring message received during sync", "msg", msg)
			return
		}
	...
	case VoteSetBitsChannel:
		conR.Logger.Debug("receive a votebit message form peer")
		if conR.WaitSync() {
			conR.Logger.Info("Ignoring message received during sync", "msg", msg)
			return
		}
	...
	//	switch msg := msg.(type) {
	//	case *PDTMessage:
	//		height, pdt_value := msg.pdt_indfo.GetValue()
	//		conR.Logger.Info("the pdt info", "height", height, "pdt_value", pdt_value)
	//	}
	default:
...
func (conR *Reactor) subscribeToBroadcastEvents() {
	...
	if err := conR.conS.evsw.AddListenerForEvent(subscriber, types.EventCommit,
		func(data tmevents.EventData) {
			conR.broadcastCommitMessage(data.(*types.PDTInfo))
		}); err != nil {
		conR.Logger.Error("Error adding listener for events", "err", err)
	}
...
func (conR *Reactor) broadcastCommitMessage(pdt_info *types.PDTInfo) {
	conR.Logger.Info("broadcastCommitMessage")
	//TODO 最好还是改成单播，而不是广播。
	conR.Switch.BroadcastEnvelope(p2p.Envelope{
		ChannelID: DataChannel,
		//这个tmcons 应该是对应的p2p中传输的消息
		Message: &tmcons.PDT{PdtInfo: pdt_info.ToProto()},
	})
	//peer_list := conR.Switch.Peers().List()
	...
	//	Message:  &tmcons.PDT{
	//		PdtInfo: pdt_info.ToProto(),
	//}, conR.Logger)
}

func (conR *Reactor) broadcastNewRoundStepMessage(rs *cstypes.RoundState) {
	nrsMsg := makeRoundStepMessage(rs)
	conR.Logger.Debug("broadcsatNewRoundStepMessage (HRS):", "Height", nrsMsg.Height, "Round", nrsMsg.Round, "step", nrsMsg.Step)
	conR.Switch.BroadcastEnvelope(p2p.Envelope{
		ChannelID: StateChannel,
		Message:   nrsMsg,
	})
	...
	//	Message:   nrsMsg,
	//})
	conR.Logger.Debug("after broadcsatNewRoundStepMessage (HRS):", "Height", nrsMsg.Height, "Round", nrsMsg.Round, "step", nrsMsg.Step)
}
	...
	for {
		...
		if rs.ProposalBlockParts.HasHeader(prs.ProposalBlockPartSetHeader) {
			if index, ok := rs.ProposalBlockParts.BitArray().Sub(prs.ProposalBlockParts.Copy()).PickRandom(); ok {
				...
				logger.Debug("start Sending block part", "height", prs.Height, "round", prs.Round, "peer_id", peer.ID())
				if p2p.SendEnvelopeShim(peer, p2p.Envelope{ //nolint: staticcheck
					...
					logger.Debug("end Sending block part", "height", prs.Height, "round", prs.Round, "peer_id", peer.ID())
		...
		if rs.Proposal != nil && !prs.Proposal {
			...
			{
				logger.Debug("start Sending proposal", "height", prs.Height, "round", prs.Round, "peer_id", peer.ID())
				if p2p.SendEnvelopeShim(peer, p2p.Envelope{ //nolint: staticcheck
					ChannelID: DataChannel,
					Message:   &tmcons.Proposal{Proposal: *rs.Proposal.ToProto()},
				}, logger) {
					// NOTE[ZM]: A peer might have received different proposal msg so this Proposal msg will be rejected!
					logger.Debug("end Sending proposal", "height", prs.Height, "round", prs.Round, "peer_id", peer.ID())
...
func (conR *Reactor) gossipDataForCatchup(logger log.Logger, rs *cstypes.RoundState,
	...
	//  logger.Info("No parts to send.sh in catch-up, sleeping")
	time.Sleep(conR.conS.config.PeerGossipSleepDuration)
}
...
OUTER_LOOP:
	for {
		...
		// If height matches, then send.sh LastCommit, Prevotes, Precommits.
		...
		// Special catchup logic.  //这个是一个特殊的追赶机制 高度只相差一个  只发送LastCommit
		// If peer is lagging by height 1, send.sh LastCommit.
		if prs.Height != 0 && rs.Height == prs.Height+1 {
			if ps.PickSendVote(rs.LastCommit) {
				logger.Debug("Picked rs.LastCommit to send.sh", "height", prs.Height)
				continue OUTER_LOOP
			}
		}

		// Catchup logic
		// If peer is lagging by more than 1, send.sh Commit.  只发送commit投票。
		blockStoreBase := conR.conS.blockStore.Base()
		if blockStoreBase > 0 && prs.Height != 0 && rs.Height >= prs.Height+2 && prs.Height >= blockStoreBase {
			...
			if commit := conR.conS.blockStore.LoadBlockCommit(prs.Height); commit != nil {
				if ps.PickSendVote(commit) {
					logger.Debug("Picked Catchup commit to send.sh", "height", prs.Height)
					continue OUTER_LOOP
				}
			}
		}
		...
		if sleeping == 0 {
			...
			//这的逻辑是到这还没有什么能够发送的。
			sleeping = 1
			logger.Debug("No votes to send.sh, sleeping", "rs.Height", rs.Height, "prs.Height", prs.Height,
...
func (conR *Reactor) gossipVotesForHeight(
	...
	// If there are lastCommits to send.sh...
	if prs.Step == cstypes.RoundStepNewHeight {
		if ps.PickSendVote(rs.LastCommit) {
			logger.Debug("Picked rs.LastCommit to send.sh")
			return true
		}
	}
	// If there are POL prevotes to send.sh...
	if prs.Step <= cstypes.RoundStepPropose && prs.Round != -1 && prs.Round <= rs.Round && prs.ProposalPOLRound != -1 {
		if polPrevotes := rs.Votes.Prevotes(prs.ProposalPOLRound); polPrevotes != nil {
			if ps.PickSendVote(polPrevotes) {
				logger.Debug("Picked rs.Prevotes(prs.ProposalPOLRound) to send.sh",
					"round", prs.ProposalPOLRound)
				return true
			}
		}
	}
	// If there are prevotes to send.sh...
	if prs.Step <= cstypes.RoundStepPrevoteWait && prs.Round != -1 && prs.Round <= rs.Round {
		if ps.PickSendVote(rs.Votes.Prevotes(prs.Round)) {
			logger.Debug("Picked rs.Prevotes(prs.Round) to send.sh", "round", prs.Round)
			return true
		}
	}
	// If there are precommits to send.sh...
	if prs.Step <= cstypes.RoundStepPrecommitWait && prs.Round != -1 && prs.Round <= rs.Round {
		if ps.PickSendVote(rs.Votes.Precommits(prs.Round)) {
			logger.Debug("Picked rs.Precommits(prs.Round) to send.sh", "round", prs.Round)
			return true
		}
	}
	// If there are prevotes to send.sh...Needed because of validBlock mechanism  //TODO 这个ValidBlock，这个其实已经commit了
	...
	// If there are POLPrevotes to send.sh...
...
OUTER_LOOP:
	for {
		...
		// Maybe send.sh Height/Round/Prevotes
		{
			rs := conR.getRoundState()
			prs := ps.GetRoundState()
			if rs.Height == prs.Height {
		...
		// Maybe send.sh Height/Round/Precommits
		{
			rs := conR.getRoundState()
			prs := ps.GetRoundState()
			if rs.Height == prs.Height {
		...
		// Maybe send.sh Height/Round/ProposalPOL
		{
			rs := conR.getRoundState()
			prs := ps.GetRoundState()
		...
		// Maybe send.sh Height/CatchupCommitRound/CatchupCommit.
		{
			prs := ps.GetRoundState()
...
// PickVoteToSend picks a vote to send.sh to the peer.
// Returns true if a vote was picked.
// NOTE: `votes` must be the correct Size() for the Height().
// 找一个可以发送的投票
// 首先获取vote集合的高度轮次，如果当前的投票集合已经提交，则需要改变peer节点的追改的轮次
...
// 获取ps上对应投票的一个状态，其实pol这个东西一直不是很理解
// TODO 为什么会是nil 好像是那个nil的状态是不用发的。
...
// 如果高度不同 跳过
// 如果已经改变了需要追赶的轮次，则跳过
// 如果当前轮次==需要追赶的轮次，则将precommit付给需要追赶的commit，否则，需要追赶的pre-commit置为空
func (ps *PeerState) ensureCatchupCommitRound(height int64, round int32, numValidators int) {
	...
	*/
	...
	ps.PRS.CatchupCommitRound = round //已经提交的Round
...
// 当前节点收到peer节点的msg，改变一下ps
func (ps *PeerState) ApplyNewRoundStepMessage(msg *NewRoundStepMessage) {
	ps.mtx.Lock()
	defer ps.mtx.Unlock()
	ps.logger.Debug("peer HRS: ")
	...
	if psHeight != msg.Height || psRound != msg.Round {
		ps.logger.Debug("enter new height")
	...
	if psHeight != msg.Height {
		...
		// We'll update the BitArray capacity later.  //这个只是为了追错过的commit的
...
func init() {
	...
	tmjson.RegisterType(&PDTMessage{}, "tendermint/PDT")
}
...
type PDTMessage struct {
	pdt_indfo *types.PDTInfo
}

func (m *PDTMessage) ValidateBasic() error {
	return nil
}
