...
type Tps struct {
	MI                int64     //监测的时间度量
	StartTime         time.Time //TPS统计的开始时间
	EndTime           time.Time //TPS统计的结束时间
	TxCommitNum       int64     //统计时间间隔内提交的交易数量
	CommitTxNumPerSec float64   //每秒提交的事务数
	Discretionary     float64   //vc的开销
}

// ComputeTps 计算一个MI中的tps
func (tps *Tps) ComputeTps() {

	tps.CommitTxNumPerSec = float64(tps.TxCommitNum) / float64(tps.EndTime.Sub(tps.StartTime).Seconds()-tps.Discretionary)
}

func (tps *Tps) Clear() {
	tps.TxCommitNum = int64(0)
	tps.Discretionary = 0.0
}
