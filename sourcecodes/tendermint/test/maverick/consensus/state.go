...
type State struct {
	...
	// information they send.sh to which nodes
	sw *p2p.Switch
}
...
func (cs *State) receiveRoutine(maxSteps int) {
	...
	for {
		...
		case ti := <-cs.timeoutTicker.Chan(): // tockChan:
			...
			// if the proposetimeout is relevant to the rs
			// go to the next step
...
func (cs *State) handleTimeout(ti timeoutInfo, rs cstypes.RoundState) {
	...
	case cstypes.RoundStepPropose:
		if err := cs.eventBus.PublishEventTimeoutPropose(cs.RoundStateEvent()); err != nil {
			cs.Logger.Error("Error publishing proposetimeout propose", "err", err)
		}
		cs.enterPrevote(ti.Height, ti.Round)
	case cstypes.RoundStepPrevoteWait:
		if err := cs.eventBus.PublishEventTimeoutWait(cs.RoundStateEvent()); err != nil {
			cs.Logger.Error("Error publishing proposetimeout wait", "err", err)
		}
...
func (cs *State) handleTxsAvailable() {
	...
	case cstypes.RoundStepNewHeight: // timeoutCommit phase
		...
		// +1ms to ensure RoundStepNewRound proposetimeout always happens after RoundStepNewHeight
