...
test_file = './test.csv'
test_store = open( test_file,'w')
test_writer = csv.writer(test_store)
...
def calculate_cook_distance(group):
    n = len(group)
    p = 1
    #print(group)
    ...
    for i in range(n):
        ...
        y_i = np.delete(y, i)
        b_i = np.linalg.pinv(X_i.T @ X_i) @ X_i.T @ y_i
        # 使用排除第i个观测值后的X矩阵计算整个y的预测值
        y_pared_i = X @ b_i
        # 计算rss_e
        rss_e = np.sum((y_pared - y_pared_i) ** 2)
        # 计算Cook's distance
        leverage = X[i, :] @ np.linalg.pinv(X.T @ X) @ X[i, :].T
        if mse*p!=0:
            #cook_d[i]=  residuals[i]**2 /(p*mse)
            cook_d[i] = (rss_e / (p * mse))
        else:
            cook_d[i]=inf
    #print("cook_d: ",cook_d)
    return cook_d
...
def count_non_zero_labels(bct, group_size, mid):
    # 初始化结果列表
    result = []

    # 计算分段数量
    num_segments = (len(bct) + group_size - 1) // group_size  # 向上取整

    # 遍历每个分段
    for i in range(num_segments):
        # 计算当前分段的起始和结束索引
        start_index = i * group_size
        end_index = min((i + 1) * group_size, len(bct))

        # 获取当前分段
        segment = bct[start_index:end_index]
...
def cook(bct_list, label_list, group_size, a_list, mid):
    data_points = []

    assert (len(bct_list) == len(label_list)), "Lengths of bct_list and label_list must be equal"
    for t in range(len(bct_list)):
        data_points.append(DataPoint(bct_list[t], label_list[t]))

    # Ensure we have the correct number of data points
    data_points = data_points[:len(bct_list)]
    ...
    # Ensure the length of a_list matches the number of full groups
    output_data_set = set()
    ...
    for i in range(num_full_groups):
        ...
        cook_distances = calculate_cook_distance(subgroup)

        # Sort the cook distances in descending order
        sorted_indices = np.argsort(cook_distances)[::-1]

        # 找到按照cook距离排序的前a_list[i]个
        for k in range(a_list[i]):
            index = sorted_indices[k]
            point = subgroup[index, 0]
            if np.mean(point) >= mid:  # 检查平均值是否大于等于mid
                output_data_set.add(point)
    ...
    if remaining_data_points > 0:
        ...
        # 找到按照cook距离排序的前min(a_list[-1], len(subgroup))个
        for k in range(min(a_list[-1], len(subgroup))):
            index = sorted_indices[k]
            point = subgroup[index, 0]
            if np.mean(point) >= mid:  # 检查平均值是否大于等于mid
                output_data_set.add(point)
    ...
    intersection = set(detected_outliers) & delay_greater_than_zero_data_set



    recall = len(intersection) / count_delay_greater_than_zero if count_delay_greater_than_zero > 0 else 0
    precision = len(intersection) / len(detected_outliers) if len(detected_outliers) > 0 else 0



    detected_outliers = sorted([int(x) for x in detected_outliers])


    return detected_outliers,recall, precision



def assign_labels(data_list):
    # 对数据进行排序
    ...
    # 根据阈值分配标签
    labels = [1 if x >= threshold_value else 0 for x in sorted_data]

    return labels



def cook_d(list1,list2,group_size,percent):
    l_sort = sorted(list1)
    index = int(2 * len(l_sort) / 3)
    mid = l_sort[index]


    a = count_non_zero_labels(list1, group_size, mid)
    # print("group_size = ", group_size)
    i = math.ceil(percent / 100 * group_size)
    number = i
    for i in range(len(a)):
        a[i] = number
    #print(round(100 * number / group_size, 2), "%")
    return cook(list1, list2, group_size, a, mid)
...
#     #read_data_into_lists("input5.txt")
#     cook_d(bct_list,label_list,percent,len(bct_list))
#
# #调用cook函数即可完成，共四个参数，分别是传入的bct_list,label_list,自己设置的阈值,和宽度group_size
# #返回一个  return detect_array,recall,precision
...
class Model:
    ...
    def __init__(self,version,height,mi,learning_model,init_timeout,input_data,cold_start_phase,experiences_window,method,para_a):
        ...
        self.Method = method        #选择的方法  ["mal-para mal-to Jac Fixed"]
        self.Para_a = para_a
    ...
    def savedataofheight(self,height_data):
        ...
        save_data.append(height_data.tps)
        # test_max_delay_time = 0.0
        # if height_data.max_delay_time > 0.0:
        test_max_delay_time = height_data.max_delay_time
        save_data.append(test_max_delay_time)
        test_delay_time = 0.0
        if height_data.delay_time >0.0:
    ...
    def initLearningModel(self,node_type):
        ...
        self.hight_data_writer.writerow(["height", "is_leader","proposer", "is_byz", "is_crash","round", "bct", "pdt","timeout", "byz_timeout", "vc-cost", "threat",
                  "discretionary", "cost", "crash", "reward", "latency", "tx_num", "tps","max_delay_time","delay_time","min","max","avg","std","deliver_time"])
        self.hight_data_store.flush()
        ...
        self.cook_Info = open(self.index + node_type + 'cook.csv', mode='w', newline='')
        self.cook_Info_writer = csv.writer(self.cook_Info)
        self.cook_Info_writer.writerow(['height','lable','pre_lable'])
        self.cook_Info.flush()
        ...
        if self.Method == 'mal-to':
            alpha_list = [0]
            beta_list = [0]
            k_list = np.linspace(0,3000,num=9*9*9+1)
        if self.Method == 'Jac':
            alpha_list = [0.125]
            beta_list = [0.25]
            k_list = [4]
    ...
    def dataPoisoning(self,bct,delay_time,state,action):
        self.Mode = True
        #作恶的几种操作
        delay_time_list = np.linspace(0, delay_time, 5)
        byz_bct_list = []
        for kk in range(len(delay_time_list)):
            byz_bct_list.append(bct+delay_time_list[len(delay_time_list)-kk-1])
        ...
        if self.Height > self.cold_start_phase+1:
            ...
            for byz_bct in byz_bct_list:
                ...
                timeout_sum = self.outer(byz_bct,testing_set)
    ...
    def defense(self,byz_node_flag,is_leader_flag):
        ...
        if self.Height > 4:
            for i in range(4):
                    if str(self.Height-i) not in self.input_data_list:
                        logging.info("dfdfddddddddd")
                    test_bct = self.input_data_list[str(self.Height-i)].BCT
                    bct_list.append(test_bct)
                    bct_lable.append(0)
            logging.info("len(bct_list): %s, %s,%s,%s",len(bct_list),bct_list,len(bct_lable),bct_lable)
            assert (len(bct_list)==len(bct_lable)==4)
            test_list,_,_ = cook_d(bct_list,bct_lable,4,50)
    ...
    #内部优化新生成的模型的效果
    def outer(self,byz_bct,testing_set):
        timeout_sum = 0
        for sample in testing_set:
            ...
            test_str = []
            if self.Height >100:
                test_str.append(str(self.Height))
                test_str.append(str(sample.Height))
                test_str.append(byz_bct)
                test_str.extend(action.tolist())
                test_str.append(str(new_timeout))
                test_writer.writerow(test_str)
                test_store.flush()
            timeout_sum = timeout_sum + new_timeout
        return timeout_sum/3
    ...
    def get_testing_set(self):
        ...
        random_numbers = [random.randint(self.MI+1, self.Height-1) for _ in range(int(self.Height)//int(20) + 1)]
    ...
    def new_height(self,bct_data,pdt_data,state,action,byz_node_flag,is_leader_flag,last_latency,min_value,max_value,avg_value,std_value):
        ...
        # test_flag = self.defense(byz_node_flag,is_leader_flag)
        test_flag = False
        ...
        if byz_node_flag and is_leader_flag:
            actul_lable = True
        self.cook_Info_writer.writerow([self.Height,actul_lable,test_flag])
        self.cook_Info.flush()
...
def generateNewHeightData(height):
    height_data = HeightData(height=height,is_leader=0,proposer=0,is_byz_node=0,is_crash_node=0,round=0,actaul_timeout=0,
                             pdt=0,predict_timeout=0,byz_timeout=0,tps = 0,latency=0,tx_num=0,max_delay_time=0.0,delay_time= 0,
...
def print_data(data,data2,label,label2,title):
    plt.plot(data,label= label,linewidth=3)
    plt.plot(data2,label= label2)

    # plt.yscale('log', base=10)  # 设置y轴为以2为底的对数坐标
...
def test():
    test_writer.writerow(["height","sample_height",'byz_bct' ,'alpha','beta','k','timeout'])
    test_store.flush()
    crash = False
    bct_list = geneateBCT()
    bct_list = read_col_data('../../../github.com/consensus/tendermint/analyze_icde/fixed-crash-32/fixed-crash-32-0/node0/honest_height_data.csv',6)
    # bct_list = read_col_data('./output/column_0.csv',1)
    # print("bct_list",bct_list)
    # test_bct_list  = []
    bct_list = bct_list[:300]
    # for tt in bct_list:
    #     test_bct_list.append(tt*10)
    # bct_list = test_bct_list
    MI = 1  #每个MI的大小
    next_byz_timeout = 5000.0  #初始化的timeout
    next_honest_timeout = 5000.0
    cold_start_phase = 30#冷启动阶段的长度
    experiences_window = 100  #经验池的大小
    method = "mal-para"
    para_a = -2
    # 这个是诚实节点设置偏大的TOu
    ...
    for i in range(len(bct_list)):
        ...
        if (i + 1) % 16 == 1:
            if i < cold_start_phase :
                max_delay_time = 2000.0
                delay_time = model_with_posioned.dataPoisoning(next_honest_timeout,max_delay_time,state_with_posioned,action_with_posioned)
                # delay_time = (1/2)*max_delay_time
                byz_height_data.max_delay_time = max_delay_time
                ...
                max_delay_time = next_byz_timeout-next_honest_timeout
                logging.info("before datapoisoning dalay_time: %s",max_delay_time)
                ...
                if delay_time < 0.0:
                    ...
                    delay_time = model_with_posioned.dataPoisoning(next_honest_timeout,max_delay_time,state_with_posioned,action_with_posioned)
                    # delay_time = (1/2)*max_delay_time
                logging.info("after datapoisoning dalay_time: %s",delay_time)
                if crash :
                    delay_time = 0.0
                byz_height_data.max_delay_time = max_delay_time
                byz_height_data.delay_time = delay_time
            # if byz_height_data.delay_time > 0.0:
                # byz_node_flag =  True
                # is_leader_flag = True
        ...
        logging.info("byz_height_data.delay_time: %s %s %s",byz_height_data.delay_time,bct_data,byz_bct_data)
        ...
        state_with_posioned,action_with_posioned = model_with_posioned.new_height(bct_data=byz_bct_data,pdt_data=byz_pdt,state=state_with_posioned,action=action_with_posioned,byz_node_flag=byz_node_flag,is_leader_flag=is_leader_flag,last_latency=byz_height_data.latency,min_value=byz_height_data.min,max_value=byz_height_data.max,avg_value=byz_height_data.actaul_timeout,std_value=byz_height_data.std)
        state_without_posioned,action_without_posioned = model_without_posioned.new_height(bct_data=bct_data,pdt_data=pdt,state=state_without_posioned,action=action_without_posioned,byz_node_flag=byz_node_flag,is_leader_flag=is_leader_flag,last_latency=honest_height_data.latency,min_value=honest_height_data.min,max_value=honest_height_data.max,avg_value=honest_height_data.actaul_timeout,std_value=honest_height_data.std)
...
if __name__ == '__main__':
   ...
   byz_bct_list,byz_to_list,byz_pdt_list = read_bct_to("1-4.csv")
   vc = read_col_data('honest_height_data.csv',5)
   tps = read_col_data('honest_height_data.csv',7)
   byz_tps = read_col_data('1-4.csv', 7)
   byz_vc = read_col_data('1-4.csv', 5)
   ...
   vc_num = sum(vc)
   avg_to = statistics.mean(to_list[30:])
   avg_to = f"{avg_to:.2f}"
   ...
   avg_byz_to = statistics.mean(byz_to_list[30:])
   avg_byz_to = f"{avg_byz_to:.2f}"

   delay_time = read_col_data('1-4.csv', 19)
   chazhi3 = []
   for i in range(len(delay_time)):
       if i+30 >= len(delay_time):
           break
       chazhi3.append(delay_time[i+30])
   ...
   print_data(bct_list,to_list,"BCT","TO","to-bct="+str(avg_Difference)+"ms  "+"avg_pdt="+str(avg_bct)+"ms  "+"vc_num="+str(vc_num)+"avg_to"+str(avg_to))
   print_data(byz_bct_list,byz_to_list,"Byz_BCT","Byz_TO","Byz_to-Byz_bct="+str(avg_Difference2)+"ms  "+"avg_Byz_pdt="+str(avg_byz_bct)+"ms  "+"Byz_vc_num="+str(byz_vc_num)+"avg_to"+str(avg_byz_to))
