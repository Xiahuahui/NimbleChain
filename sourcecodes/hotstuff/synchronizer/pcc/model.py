...
class Model:
    ...
    def __init__(self,ID,version,height,mi,learning_model,init_timeout,input_data,cold_start_phase,experiences_window,method,para_a):
#       id = os.getenv("ID")
#         self.index = '../config_path/node'+id+'/'
    ...
    def get_State(self,start_height):
        ...
        for t in range(self.MI):
            if str(start_height-t) in self.input_data_list:
                ...
                raise Exception("the height of "+str(start_height-t)+ " input_data is not in the input_data_list1")
    ...
    def get_MI_info(self,start_height):
        ...
        logging.info(f"worker {os.getpid()} : the start_height is: {start_height}")
        for t in range(self.MI):
            if str(start_height-t) in self.input_data_list:
                ...
                raise Exception("ID:"+os.getpid()+" the height of "+str(start_height-t)+ " input_data is not in the input_data_list2")
    ...
    def getMIBCTList(self,start_height):
        ...
        for t in range(self.MI):
            if str(start_height-t) in self.input_data_list:
                ...
                raise Exception("the height of "+str(start_height-t)+ " input_data is not in the input_data_list3")
        return bct_list
    ...
    def getMIPDTList(self,start_height):
        ...
        for t in range(self.MI):
            if str(start_height-t) in self.input_data_list:
                ...
                raise Exception("the height of "+str(start_height-t)+ " input_data is not in the input_data_list4")
        return pdt_list
    ...
    def getMITOList(self,start_height):
        ...
        for t in range(self.MI):
            if str(start_height-t) in self.input_data_list:
                ...
                raise Exception("the height of "+str(start_height-t)+ " input_data is not in the input_data_list5")
        return to_list
...
# 线性归一化
def linear_normalization(value, min_value=-3000.0, max_value=3000.0, a=-5, b=5):
    if value < -990:
        value = -990.0
    if value > 1800.0:
        value = 1800.0
    ...
    normalized_value = 0.000553 * value - 0.0517
...
def transform(x, a):
    ...
    a=-0.2
    return 9 / (math.exp(x + a)+ math.exp(-x -a))**2
...
# 第一高度上是没有state的
#
def generatemodel(id,node_type,version,MI,init_timeout,cold_start_phase,experiences_window,method,para_a):
    ...
    model = Model(ID=id,version=version,height=height,mi=MI,learning_model=learning_model,init_timeout=init_timeout,input_data=input_data,cold_start_phase=cold_start_phase,experiences_window=experiences_window,method=method,para_a=para_a)
...
def test():
    ...
    model_with_posioned,state_with_posioned,action_with_posioned = generatemodel(0,'byz',version="v3",MI=MI,init_timeout=next_byz_timeout,cold_start_phase=cold_start_phase,experiences_window = experiences_window,method=method,para_a=para_a)

    model_without_posioned,state_without_posioned,action_without_posioned = generatemodel(0,'honest',version="v3",MI=MI,init_timeout=next_honest_timeout,cold_start_phase=cold_start_phase,experiences_window = experiences_window,method=method,para_a=para_a)
