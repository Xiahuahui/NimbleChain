...
class PccServicer(pcc_pb2_grpc.PccServiceServicer):
    ...
    def InitPCC(self, request, context):
        ...
        self.is_byz_flag = request.is_byz                              
        ...
        logging.info("the is a malicious node? %s , the could_start_phase'len is: %s the version is: %s,para_a",self.is_byz_flag,request.cold_start_phase,self.version,para_a)
        # 这个是诚实节点设置偏大的TOu

        model,state,action = m.generatemodel(self.id,'honest',version=self.version,MI=MI,init_timeout=init_timeout,cold_start_phase=cold_start_phase,experiences_window = experiences_window,method=method,para_a=para_a)
        self.model_with_posioned = model              #每种节点都有
        self.action_with_posioned = action
        self.state_with_posioned = state

        # if self.is_byz_flag:
        model_byz,state_byz,action_byz = m.generatemodel(self.id,'byz',version=self.version,MI=MI,init_timeout=init_timeout,cold_start_phase=cold_start_phase,experiences_window= experiences_window,method=method,para_a=para_a)  #这个是拜占庭节点估计出来的正常的网络延迟
...
def start_server(id):
    ...
    pcc_pb2_grpc.add_PccServiceServicer_to_server(PccServicer(id), server)
    server.add_insecure_port('localhost:500'+str(id + 11))
    server.start()
    logging.info("the pcc service is starting ,the service's adderss:port is localhost:500"+str(id + 11))
...
def serve():
    ...
    for i in range (thread_num):
        p = Process(target=start_server, args=(i,))
        p.start()
        processes.append(p)
