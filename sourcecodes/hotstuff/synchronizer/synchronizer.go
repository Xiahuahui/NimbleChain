...
func (s *Synchronizer)CheckTimeout(view hotstuff.View) bool{
	if s.is_timeout && view <= s.View()+1 {
		return true
	}else{
		return false
	}
...
func (s *Synchronizer) OnLocalTimeout() {
	...
	s.duration.ViewTimeout(s.View()) // increase the duration of the next view
...
func (s *Synchronizer) UpdateLeaderTimer(){
	s.duration.ViewSucceeded(s.View(),s.opts.ID())
	s.duration.ViewStarted()
}

func (s *Synchronizer)GetTimeout() float64{
	return s.duration.GetTimeout()
}
...
func (s *Synchronizer) AdvanceView(syncInfo hotstuff.SyncInfo) {
	...
	if !timeout {
		s.duration.ViewSucceeded(s.currentView,s.leaderRotation.GetLeader(v+1))
	}
	...
	//ways to skip consecutive timeout if there are some crashed nodes
	// if timeout{
	// 	newView++
	...
	// 	} else if replica, ok := s.configuration.Replica(s.leaderRotation.GetLeader(newView-1)); ok {
	...
	//easily get consecutive timeouts if there are some crashed nodes
	leader := s.leaderRotation.GetLeader(newView)
		if leader == s.opts.ID() {
			s.currentView = newView
			s.consensus.Propose(syncInfo)
