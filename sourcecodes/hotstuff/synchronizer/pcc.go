...
import (
	"context"
	"fmt"
	pb "github.com/relab/hotstuff/synchronizer/proto/proto"
...
// 初始化pcc
func InitPCC(IDaddress int64, mi int64, smi int64, epoch_size int64, timeout_value int64, is_byz bool, is_crash bool, cold_start_phase int64, version string, delay_time float64, method string, para_a float64) PCCTimeout {
	address := "localhost:500" + strconv.Itoa(int(IDaddress)+10)
...
// TODO 做一些清除工作
func (pcct *PCCTimeout) RunRLJ(height int64, delay_time float64, BCT float64, PDT float64, is_crash bool,is_leader bool,is_byz bool, min float64, max float64, avg float64, std float64, deliver_time float64,proposer int64) (int64, float64, float64, float64) {
	rlj_req := pb.RunRLJRequest{}
	rlj_req.HeightData = &pb.HeightData{}
	rlj_req.HeightData.Pdt = PDT
	rlj_req.HeightData.IsCrashNode = is_crash
	rlj_req.HeightData.Min = min
	rlj_req.HeightData.Height = height
	rlj_req.HeightData.IsLeader = is_leader        //当前节点是不是leader
	rlj_req.HeightData.Proposer = proposer           //哪个节点是leader
	rlj_req.HeightData.IsByzNode = is_byz      //当前节点是不是拜占庭节点
	rlj_req.HeightData.ActaulTimeout = BCT     //bct的值
	rlj_req.HeightData.PredictTimeout = 3000.0 //当前view的timeout
	rlj_req.HeightData.ByzTimeout = 3000.0     //拜占庭节点的to
	rlj_req.HeightData.Round = 0
	rlj_req.HeightData.Latency = 100.0                                                      //当前区块的延迟
	rlj_req.HeightData.TxNum = 10000                                                        //当前区块中交易数量
	rlj_req.HeightData.Tps = float64(rlj_req.HeightData.TxNum) / rlj_req.HeightData.Latency //当前区块中交易数量
	rlj_req.HeightData.DelayTime = delay_time                                               //当前高度的延迟时间
	rlj_req.HeightData.DeliverTime = deliver_time                                           //当前节点的发送时间
