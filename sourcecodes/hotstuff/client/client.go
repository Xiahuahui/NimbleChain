...
loop:
	for {
		...
		//c.logger.Info("before wait")
		err := c.limiter.Wait(ctx)
		//c.logger.Info("after wait")
		...
		if shouldStop {
			c.logger.Infof("break")
			break
		} //如果lastcommmand比highestCommitted小，那么就退出循环？？
		...
		//c.logger.Infof("%d commands ready to sent", num)
		...
		//c.logger.Infof("%d commands received", num)
		//c.logger.Info("loop end_1")
		num++
		//c.logger.Info("channel size: ", len(c.pendingCmds))
		select {
		case c.pendingCmds <- pending:
			//c.logger.Info("reply ", num, "sent to handler ")
		case <-ctx.Done():
			break loop
		}
...
func (c *Client) handleCommands(ctx context.Context) (executed, failed, timeout int) {
	...
	for {
		...
		case cmd, ok = <-c.pendingCmds:
			//c.logger.Info("reply get ", num)
			num++
			if !ok {
				c.logger.Info("channel send error")
				return
			}
		case <-ctx.Done():
			return
