...
using the '--host' parameter. This should be a comma separated list of hostnames or ip addresses.`,
	Run: func(_ *cobra.Command, _ []string) {
		fmt.Println("begin run controller")
		runController()
	},
}
...
func init() {
	...
	runCmd.Flags().Int("replicas", 32, "number of replicas to run")
	...
	runCmd.Flags().Int("max-concurrent", 10, "maximum number of concurrent commands per client")
	runCmd.Flags().Duration("client-timeout", 500*time.Millisecond, "Client timeout.")
	runCmd.Flags().Duration("duration", 200*time.Second, "duration of the experiment")
	runCmd.Flags().Duration("connect-timeout", 5*time.Second, "duration of the initial connection timeout")
	runCmd.Flags().Duration("view-timeout", 3000*time.Millisecond, "duration of the first view")
	...
	runCmd.Flags().Float32("timeout-multiplier", 2, "number to multiply the view duration by in case of a timeout")
